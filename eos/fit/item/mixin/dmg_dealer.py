...
class DmgDealerMixin(BaseItemMixin):
    ...
    def __get_volley_self(self):
        """Return item damage attributes as 4-tuple."""
        return self.__get_volley_item(self)

    def __get_volley_charge(self):
        """Return item's charge damage attributes as 4-tuple."""
        charge = getattr(self, 'charge', None)
        if charge is None:
    ...
    def __get_volley_hybrid(self):
        """If charge is loaded, return damage attributes, else item's."""
        charge = getattr(self, 'charge', None)
    ...
    def __get_volley_item(self, item):
        ...
        therm = item.attrs.get(AttrId.thermal_dmg)
        kin = item.attrs.get(AttrId.kinetic_dmg)
        expl = item.attrs.get(AttrId.explosive_dmg)
    ...
    def _pereff_volleys(self):
        ...
        for effect_id, weapon_type in self._pereff_weapon_types.items():
            ...
            if multiply:
                try:
                    multiplier = self.attrs[AttrId.dmg_multiplier]
                except KeyError:
                    pass
                else:
    ...
    def get_nominal_volley(self, tgt_resists=None):
        ...
        Args:
            tgt_resists (optional): ResistProfile helper container instance. If
                specified, effective damage against these resistances is
                calculated.
        ...
        for volley in self._pereff_volleys.values():
            ...
            if expl is None:
                ...
                try:
                    ...
                    pass
        if tgt_resists:
            em_resonance = 1 - tgt_resists.em
            therm_resonance = 1 - tgt_resists.thermal
            kin_resonance = 1 - tgt_resists.kinetic
            expl_resonance = 1 - tgt_resists.explosive

...
class TestSourceAttrNone(AttrCalcTestCase):
    ...
    def setUp(self):
        ...
        invalidInfo.sourceValue = noneAttr.id
        self.effect = Effect(None, EffectCategory.passive)
        self.fit = Fit({tgtAttr.id: tgtAttr, noneAttr.id: noneAttr, srcAttr.id: srcAttr})

    def testLog(self):
        self.effect._infos = (self.invalidInfo,)
        holder = IndependentItem(Type(529, effects=(self.effect,), attributes={self.tgtAttr.id: 100, self.noneAttr.id: None}))
        self.fit.items.append(holder)
        self.assertAlmostEqual(holder.attributes[self.tgtAttr.id], 100)
        # First error notifies about base value of noneAttr
    ...
    def testCombination(self):
        ...
        holder = IndependentItem(Type(None, effects=(self.effect,), attributes={self.srcAttr.id: 1.5, self.tgtAttr.id: 100, self.noneAttr.id: None}))
        self.fit.items.append(holder)

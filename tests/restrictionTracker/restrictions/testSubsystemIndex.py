...
class TestSubsystemIndex(RestrictionTestCase):
    ...
    def testFail(self):
        ...
        item = self.ch.type_(typeId=1, attributes={Attribute.subSystemSlot: 120})
        ...
        restrictionError1 = self.getRestrictionError(holder1, Restriction.subsystemIndex)
        self.assertIsNotNone(restrictionError1)
        self.assertEqual(restrictionError1.holderSlotIndex, 120)
        restrictionError2 = self.getRestrictionError(holder2, Restriction.subsystemIndex)
    ...
    def testPass(self):
        ...
        restrictionError = self.getRestrictionError(holder, Restriction.subsystemIndex)
    ...
    def testPassDifferent(self):
        ...
        item1 = self.ch.type_(typeId=1, attributes={Attribute.subSystemSlot: 120})
        item2 = self.ch.type_(typeId=2, attributes={Attribute.subSystemSlot: 121})
        holder1 = Mock(state=State.offline, item=item1, _location=Location.ship, spec_set=Subsystem)
        holder2 = Mock(state=State.offline, item=item2, _location=Location.ship, spec_set=Subsystem)
        self.trackHolder(holder1)
        self.trackHolder(holder2)
        restrictionError1 = self.getRestrictionError(holder1, Restriction.subsystemIndex)

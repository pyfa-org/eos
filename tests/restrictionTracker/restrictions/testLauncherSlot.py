...
class TestLauncherSlot(RestrictionTestCase):
    ...
    def testFail(self):
        ...
        fit = Fit()
        item = Type(None)
        item._Type__slots = {Slot.launcher}
    ...
    def testFailShipAttrNone(self):
        ...
        ship = IndependentItem(Type(None))
        ship.attributes[Attribute.launcherSlotsLeft] = None
        fit.ship = ship
    ...
    def testFailModified(self):
        ...
        holder2 = ShipItem(item)
        fit.items.append(holder2)
        ship = IndependentItem(Type(None, attributes={Attribute.launcherSlotsLeft: 5}))
        ship.attributes[Attribute.launcherSlotsLeft] = 1
        fit.ship = ship
        restrictionError1 = fit.getRestrictionError(holder1, Restriction.launcherSlot)
        self.assertIsNotNone(restrictionError1)

...
class TestLauncherSlot(RestrictionTestCase):
    ...
    def testFail(self):
        ...
        shipHolder.attributes = {Attribute.launcherSlotsLeft: 1}
    ...
    def testFailModified(self):
        ...
        holder2 = Mock(state=State.offline, item=item, _location=Location.ship, spec_set=Module)
        self.trackHolder(holder2)
        shipItem = self.ch.type_(typeId=2, attributes={Attribute.launcherSlotsLeft: 5})
        shipHolder = Mock(state=State.offline, item=shipItem, _location=None, spec_set=Ship)
        shipHolder.attributes = {Attribute.launcherSlotsLeft: 1}
        self.setShip(shipHolder)
        restrictionError1 = self.getRestrictionError(holder1, Restriction.launcherSlot)
    ...
    def testPass(self):
        ...
        shipItem = self.ch.type_(typeId=2)
        shipHolder = Mock(state=State.offline, item=shipItem, _location=None, spec_set=Ship)
        shipHolder.attributes = {Attribute.launcherSlotsLeft: 3}
        self.setShip(shipHolder)
        restrictionError1 = self.getRestrictionError(holder1, Restriction.launcherSlot)

...
class TestLaunchedDrone(RestrictionTestCase):
    ...
    def testFailNoAttr(self):
        ...
        fit.character = char
        restrictionError = fit.getRestrictionError(holder1, Restriction.launchedDrone)
        self.assertIsNotNone(restrictionError)
        self.assertEqual(restrictionError.maxLaunchedDrones, 1)
        self.assertEqual(restrictionError.launchedDrones, 2)
    ...
    def testFailExcessOriginal(self):
        ...
        holder2.state = State.online
        fit.drones.append(holder2)
        char = IndependentItem(Type(None, attributes={Attribute.maxActiveDrones: 3}))
        char.attributes[Attribute.maxActiveDrones] = 1
        fit.character = char
        restrictionError = fit.getRestrictionError(holder1, Restriction.launchedDrone)
        self.assertIsNotNone(restrictionError)

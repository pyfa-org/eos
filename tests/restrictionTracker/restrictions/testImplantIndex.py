...
class TestImplantIndex(RestrictionTestCase):
    ...
    def testPass(self):
        ...
        # trigger any errors
        fit = Fit()
        holder = IndependentItem(Type(None, attributes={Attribute.implantness: 120}))
    ...
    def testPassDifferent(self):
        ...
        holder1 = IndependentItem(Type(None, attributes={Attribute.implantness: 120}))
        fit.items.append(holder1)
        holder2 = IndependentItem(Type(None, attributes={Attribute.implantness: 121}))
        fit.items.append(holder2)
        restrictionError1 = fit.getRestrictionError(holder1, Restriction.implantIndex)
    ...
    def testPassAttrNone(self):
        ...
        # should be generated
        fit = Fit()
        holder1 = IndependentItem(Type(None, attributes={Attribute.implantness: None}))
        fit.items.append(holder1)
        holder2 = IndependentItem(Type(None, attributes={Attribute.implantness: None}))
        fit.items.append(holder2)
        restrictionError1 = fit.getRestrictionError(holder1, Restriction.implantIndex)
        self.assertIsNone(restrictionError1)
        restrictionError2 = fit.getRestrictionError(holder2, Restriction.implantIndex)

...
class TestSubsystemSlot(RestrictionTestCase):
    ...
    def testFail(self):
        ...
        fit = Fit()
        item = Type(None)
        item._Type__slots = {Slot.subsystem}
    ...
    def testFailShipAttrNone(self):
        ...
        ship = IndependentItem(Type(None))
        ship.attributes[Attribute.maxSubSystems] = None
        fit.ship = ship
        restrictionError = fit.getRestrictionError(holder, Restriction.subsystemSlot)
    ...
    def testFailModified(self):
        ...
        holder2 = ShipItem(item)
        fit.items.append(holder2)
        ship = IndependentItem(Type(None, attributes={Attribute.maxSubSystems: 5}))
        ship.attributes[Attribute.maxSubSystems] = 1
        fit.ship = ship
        restrictionError1 = fit.getRestrictionError(holder1, Restriction.subsystemSlot)
        self.assertIsNotNone(restrictionError1)
    ...
    def testPass(self):
        ...
        fit.items.append(holder2)
        ship = IndependentItem(Type(None))
        ship.attributes[Attribute.maxSubSystems] = 3
        fit.ship = ship
        restrictionError1 = fit.getRestrictionError(holder1, Restriction.subsystemSlot)

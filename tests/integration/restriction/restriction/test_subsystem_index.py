...
class TestSubsystemIndex(RestrictionTestCase):
    ...
    def test_fail(self):
        ...
        fit = Fit()
        eve_type = self.ch.type(attributes={Attribute.subsystem_slot: 120})
    ...
    def test_pass_different(self):
        ...
        fit = Fit()
        eve_type1 = self.ch.type(attributes={Attribute.subsystem_slot: 120})
        eve_type2 = self.ch.type(attributes={Attribute.subsystem_slot: 121})
        item1 = Subsystem(eve_type1.id)
        item2 = Subsystem(eve_type2.id)
        self.add_item(item1)
        self.add_item(item2)
        restriction_error1 = self.get_restriction_error(fit, item1, Restriction.subsystem_index)

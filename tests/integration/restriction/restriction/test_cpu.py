...
class TestCpu(RestrictionTestCase):
    ...
    def test_fail_excess_single(self):
        ...
        fit = Fit()
        eve_type = self.ch.type(attributes={Attribute.cpu: 0})
        item = ModuleHigh(eve_type.id, state=State.online)
        item.attributes = {Attribute.cpu: 50}
        self.add_item(item)
        fit.stats.cpu.used = 50
        fit.stats.cpu.output = 40
    ...
    def test_fail_excess_single_undefined_output(self):
        ...
        eve_type = self.ch.type(attributes={Attribute.cpu: 0})
        item = ModuleHigh(eve_type.id, state=State.online)
        item.attributes = {Attribute.cpu: 5}
        self.add_item(item)
        fit.stats.cpu.used = 5
        fit.stats.cpu.output = None
        restriction_error = self.get_restriction_error(fit, item, Restriction.cpu)
        self.assertIsNotNone(restriction_error)
    ...
    def test_fail_excess_multiple(self):
        ...
        item1.attributes = {Attribute.cpu: 25}
        self.add_item(item1)
        item2 = ModuleHigh(eve_type.id, state=State.online)
        item2.attributes = {Attribute.cpu: 20}
        self.add_item(item2)
        fit.stats.cpu.used = 45
        fit.stats.cpu.output = 40
    ...
    def test_fail_excess_modified(self):
        ...
        fit = Fit()
        eve_type = self.ch.type(attributes={Attribute.cpu: 40})
        item = ModuleHigh(eve_type.id, state=State.online)
        item.attributes = {Attribute.cpu: 100}
        self.add_item(item)
        fit.stats.cpu.used = 100
        fit.stats.cpu.output = 50
        restriction_error = self.get_restriction_error(fit, item, Restriction.cpu)
        self.assertIsNotNone(restriction_error)
    ...
    def test_mix_usage_negative(self):
        # If some item has negative usage and cpu error is
        # still raised, check it's not raised for item with
        # negative usage
        fit = Fit()
        eve_type = self.ch.type(attributes={Attribute.cpu: 0})
        item1 = ModuleHigh(eve_type.id, state=State.online)
        item1.attributes = {Attribute.cpu: 100}
        self.add_item(item1)
        item2 = ModuleHigh(eve_type.id, state=State.online)
        item2.attributes = {Attribute.cpu: -10}
        self.add_item(item2)
        fit.stats.cpu.used = 90
        fit.stats.cpu.output = 50
        restriction_error1 = self.get_restriction_error(fit, item1, Restriction.cpu)
        self.assertIsNotNone(restriction_error1)
    ...
    def test_mix_usage_zero(self):
        ...
        self.add_item(item1)
        item2 = ModuleHigh(eve_type.id, state=State.online)
        item2.attributes = {Attribute.cpu: 0}
        self.add_item(item2)
        fit.stats.cpu.used = 100
        fit.stats.cpu.output = 50
        restriction_error1 = self.get_restriction_error(fit, item1, Restriction.cpu)

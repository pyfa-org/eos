...
class TestChargeVolume(RestrictionTestCase):
    ...
    def test_fail_greater(self):
        ...
        container_item = ModuleHigh(self.ch.type(
            attributes={AttributeId.capacity: 1}).id, state=State.offline)
    ...
    def test_pass_no_volume(self):
        ...
        container_item = ModuleHigh(self.ch.type(
            attributes={AttributeId.volume: 3}).id, state=State.offline)
        container_item.charge = charge_item
        self.fit.modules.high.append(container_item)
        # Action
        restriction_error1 = self.get_restriction_error(
    ...
    def test_pass_equal(self):
        ...
        container_item = ModuleHigh(self.ch.type(
            attributes={AttributeId.volume: 2}).id, state=State.offline)
        container_item.charge = charge_item
        self.fit.modules.high.append(container_item)
        # Action
        restriction_error1 = self.get_restriction_error(
    ...
    def test_pass_lesser(self):
        charge_item = Charge(self.ch.type(
            attributes={AttributeId.volume: 2}).id)
        container_item = ModuleHigh(self.ch.type(
            attributes={AttributeId.capacity: 3}).id, state=State.offline)
        container_item.charge = charge_item
        self.fit.modules.high.append(container_item)
        # Action
        restriction_error1 = self.get_restriction_error(

...
class TestMaxGroupActive(RestrictionTestCase):
    ...
    def test_mix_excess_one(self):
        ...
        fit = Fit()
        eve_type1 = self.ch.type(group=92, attributes={Attribute.max_group_active: 1})
        item1 = ModuleHigh(eve_type1.id, state=State.active)
        self.add_item(item1)
        eve_type2 = self.ch.type(group=92, attributes={Attribute.max_group_active: 2})
        item2 = ModuleHigh(eve_type2.id, state=State.active)
        self.add_item(item2)
        # Action
        restriction_error1 = self.get_restriction_error(fit, item1, Restriction.max_group_active)
        # Verification
    ...
    def test_mix_excess_attr_eve_type(self):
        ...
        fit = Fit()
        eve_type1 = self.ch.type(group=61, attributes={Attribute.max_group_active: 1})
        item1 = ModuleHigh(eve_type1.id, state=State.active)
        item1.attributes = {Attribute.max_group_active: 2}
        self.add_item(item1)
        eve_type2 = self.ch.type(group=61, attributes={Attribute.max_group_active: 2})
        item2 = ModuleHigh(eve_type2.id, state=State.active)
    ...
    def test_pass_item_other_class(self):
        ...
        item1 = Rig(eve_type.id, state=State.active)
        self.add_item(item1)
        item2 = Rig(eve_type.id, state=State.active)
        self.add_item(item2)
        # Action
        restriction_error1 = self.get_restriction_error(fit, item1, Restriction.max_group_active)
        # Verification

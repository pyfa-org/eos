...
class TestFighterSquadLight(RestrictionTestCase):
    """Check functionality of light fighter squad quantity restriction."""
    ...
    def test_fail_single(self):
        ...
        item = FighterSquad(self.mktype(
            attrs={AttrId.fighter_squadron_is_light: 1}).id)
    ...
    def test_fail_multiple(self):
        ...
        self.fit.ship = Ship(self.mktype(
            attrs={AttrId.fighter_light_slots: 1}).id)
    ...
    def test_pass_equal(self):
        self.fit.ship = Ship(self.mktype(
            attrs={AttrId.fighter_light_slots: 2}).id)
        item_type = self.mktype(attrs={AttrId.fighter_squadron_is_light: 1})
        item1 = FighterSquad(item_type.id)
        item2 = FighterSquad(item_type.id)
        self.fit.fighters.add(item1)
    ...
    def test_pass_greater(self):
        self.fit.ship = Ship(self.mktype(
            attrs={AttrId.fighter_light_slots: 5}).id)
        item_type = self.mktype(attrs={AttrId.fighter_squadron_is_light: 1})
        item1 = FighterSquad(item_type.id)
        item2 = FighterSquad(item_type.id)
        self.fit.fighters.add(item1)

...
class TestMaxGroupOnline(RestrictionTestCase):
    ...
    def test_fail_excess_all(self):
        ...
        # Action
        error2 = self.get_error(item2, Restriction.max_group_online)
        # Verification
    ...
    def test_mix_excess_one(self):
        ...
        item1 = ModuleHigh(
            self.mktype(group_id=92, attrs={AttrId.max_group_online: 1}).id,
            state=State.online)
        self.fit.modules.high.append(item1)
        item2 = ModuleHigh(
            self.mktype(group_id=92, attrs={AttrId.max_group_online: 2}).id,
            state=State.online)
        self.fit.modules.high.append(item2)
        # Action
        error1 = self.get_error(item1, Restriction.max_group_online)
    ...
    def test_pass(self):
        ...
        item_type = self.mktype(
            group_id=860,
            attrs={AttrId.max_group_online: 2})
        item1 = ModuleHigh(item_type.id, state=State.online)
        self.fit.modules.high.append(item1)
        item2 = ModuleHigh(item_type.id, state=State.online)
        self.fit.modules.high.append(item2)

...
class TestLowSlot(RestrictionTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.ch.attribute(attribute_id=Attribute.low_slots)
        self.slot_effect = self.ch.effect(effect_id=Effect.lo_power, category=EffectCategory.passive)
    ...
    def test_fail_excess_multiple(self):
        ...
        fit = Fit()
        fit.ship = Ship(self.ch.type(attributes={Attribute.low_slots: 1}).id)
        ...
        # Action
        restriction_error1 = self.get_restriction_error(fit, item1, Restriction.low_slot)
        # Verification
        self.assertIsNone(restriction_error1)
        # Action
        restriction_error2 = self.get_restriction_error(fit, item2, Restriction.low_slot)
        # Verification
    ...
    def test_fail_excess_multiple_with_nones(self):
        ...
        fit = Fit()
        fit.ship = Ship(self.ch.type(attributes={Attribute.low_slots: 3}).id)
        eve_type = self.ch.type(effects=(self.slot_effect,))
        item1 = ModuleLow(eve_type.id)
        item2 = ModuleLow(eve_type.id)
        item3 = ModuleLow(eve_type.id)
        fit.modules.low.place(1, item1)
        fit.modules.low.place(4, item2)
        fit.modules.low.place(6, item3)
        # Action
        restriction_error1 = self.get_restriction_error(fit, item1, Restriction.low_slot)
        # Verification
        self.assertIsNone(restriction_error1)
        ...
        # Action
        restriction_error3 = self.get_restriction_error(fit, item2, Restriction.low_slot)
        # Verification
    ...
    def test_pass_equal(self):
        fit = Fit()
        fit.ship = Ship(self.ch.type(attributes={Attribute.low_slots: 2}).id)
        eve_type = self.ch.type(effects=(self.slot_effect,))
        item1 = ModuleLow(eve_type.id)
        item2 = ModuleLow(eve_type.id)
        fit.modules.low.append(item1)
    ...
    def test_pass_greater(self):
        fit = Fit()
        fit.ship = Ship(self.ch.type(attributes={Attribute.low_slots: 5}).id)
        eve_type = self.ch.type(effects=(self.slot_effect,))
        item1 = ModuleLow(eve_type.id)
        item2 = ModuleLow(eve_type.id)
        fit.modules.low.append(item1)

...
class TestLauncherSlot(StatTestCase):
    ...
    def test_output(self):
        ...
        modifier = self.mod(
            ...
            tgt_attr=AttributeId.launcher_slots_left,
        ...
        fit.ship = Ship(self.ch.type(
            attributes={AttributeId.launcher_slots_left: 3, src_attr.id: 2}, effects=[mod_effect]
        ).id)
        # Verification
    ...
    def test_no_source(self):
        fit = Fit()
        fit.ship = Ship(self.ch.type(attributes={AttributeId.launcher_slots_left: 3}).id)
        fit.modules.high.append(ModuleHigh(self.ch.type(effects=[self.effect]).id))
        fit.modules.high.append(ModuleHigh(self.ch.type(effects=[self.effect]).id))
        fit.source = None
        # Verification

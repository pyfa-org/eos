...
class TestLowSlot(StatTestCase):
    ...
    def test_output(self):
        ...
        modifier = DogmaModifier(
            ...
            tgt_attr=Attribute.low_slots,
            operator=ModifierOperator.post_mul,
            src_attr=src_attr.id
        )
        effect = self.ch.effect(category=EffectCategory.passive, modifiers=[modifier])
        fit = Fit()
        fit.ship = Ship(self.ch.type(effects=[effect], attributes={Attribute.low_slots: 3, src_attr.id: 2}).id)
        # Verification
        self.assertAlmostEqual(fit.stats.low_slots.total, 6)
        # Cleanup
    ...
    def test_use_multiple_with_none(self):
        fit = Fit()
        fit.modules.low.place(1, ModuleLow(self.ch.type(effects=[self.slot_effect]).id))

...
class TestFighterSquadSupport(StatsTestCase):
    ...
    def test_output(self):
        ...
        modifier = self.mkmod(
            ...
            affectee_attr_id=AttrId.fighter_support_slots,
        ...
        self.fit.ship = Ship(self.mktype(
            attrs={AttrId.fighter_support_slots: 3, src_attr.id: 2},
            effects=[mod_effect]).id)
        # Verification
        self.assertEqual(self.fit.stats.fighter_squads_support.total, 6)
        # Cleanup
    ...
    def test_output_ship_absent(self):
        self.fit.ship = None
        # Verification
        self.assertEqual(self.fit.stats.fighter_squads_support.total, 0)
        # Cleanup
        self.assert_solsys_buffers_empty(self.fit.solar_system)
        self.assert_log_entries(0)
    ...
    def test_use_multiple(self):
        item_type = self.mktype(attrs={AttrId.fighter_squadron_is_support: 1.0})
        self.fit.fighters.add(FighterSquad(item_type.id))
        self.fit.fighters.add(FighterSquad(item_type.id))
        # Verification
        self.assertEqual(self.fit.stats.fighter_squads_support.used, 2)
        # Cleanup
        self.assert_solsys_buffers_empty(self.fit.solar_system)
        self.assert_log_entries(0)
    ...
    def test_use_item_attr_absent(self):
        ...
        # Verification
        self.assertEqual(self.fit.stats.fighter_squads_support.used, 0)
        # Cleanup
        self.assert_solsys_buffers_empty(self.fit.solar_system)
        self.assert_log_entries(0)
    ...
    def test_use_item_attr_zero(self):
        self.fit.fighters.add(FighterSquad(self.mktype(
            attrs={AttrId.fighter_squadron_is_support: 0.0}).id))
        # Verification
        self.assertEqual(self.fit.stats.fighter_squads_support.used, 0)
        # Cleanup
        self.assert_solsys_buffers_empty(self.fit.solar_system)
    ...
    def test_use_item_class_other(self):
        self.fit.drones.add(Drone(self.mktype(
            attrs={AttrId.fighter_squadron_is_support: 1.0}).id))
        # Verification
        self.assertEqual(self.fit.stats.fighter_squads_support.used, 0)
        # Cleanup
        self.assert_solsys_buffers_empty(self.fit.solar_system)

...
class TestItemDamageSmartbomb(ItemMixinTestCase):
    ...
    def setUp(self):
        ...
        self.effect = self.ch.effect(
            effect_id=Effect.emp_wave, category=EffectCategory.active, duration_attribute=self.cycle_attr.id
        )
    ...
    def test_nominal_volley_multiplier(self):
        ...
        item = ModuleHigh(self.ch.type(attributes={
            Attribute.em_damage: 52, Attribute.thermal_damage: 63, Attribute.kinetic_damage: 74,
            Attribute.explosive_damage: 85, self.cycle_attr.id: 5000, Attribute.damage_multiplier: 5.5
        }, effects=[self.effect], default_effect=self.effect).id, state=State.active)
        fit.modules.high.append(item)
        # Verification
        volley = item.get_nominal_volley()

...
class TestSubsystemIndex(RestrictionTestCase):
    ...
    def test_pass_different(self):
        ...
        holder1 = Mock(state=State.offline, _eve_type=eve_type1, _domain=ModifierDomain.ship, spec_set=Subsystem(1))
        holder2 = Mock(state=State.offline, _eve_type=eve_type2, _domain=ModifierDomain.ship, spec_set=Subsystem(1))
        self.add_holder(holder1)
        self.add_holder(holder2)
        restriction_error1 = self.get_restriction_error(holder1, Restriction.subsystem_index)

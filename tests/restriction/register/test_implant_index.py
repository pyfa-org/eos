...
class TestImplantIndex(RestrictionTestCase):
    ...
    def test_fail(self):
        ...
        item1 = self.make_item_mock(Implant, eve_type)
        item2 = self.make_item_mock(Implant, eve_type)
        self.add_item(item1)
        self.add_item(item2)
        restriction_error1 = self.get_restriction_error(item1, Restriction.implant_index)
        self.assertIsNotNone(restriction_error1)
        self.assertEqual(restriction_error1.item_slot_index, 120)
        restriction_error2 = self.get_restriction_error(item2, Restriction.implant_index)
    ...
    def test_pass(self):
        ...
        self.add_item(item)
        restriction_error = self.get_restriction_error(item, Restriction.implant_index)
    ...
    def test_pass_different(self):
        ...
        eve_type1 = self.ch.type(type_id=1, attributes={Attribute.implantness: 120})
        eve_type2 = self.ch.type(type_id=2, attributes={Attribute.implantness: 121})
        item1 = self.make_item_mock(Implant, eve_type1)
        item2 = self.make_item_mock(Implant, eve_type2)
        self.add_item(item1)
        self.add_item(item2)
        restriction_error1 = self.get_restriction_error(item1, Restriction.implant_index)

...
class TestSubsystemIndex(RestrictionTestCase):
    ...
    def test_pass_different(self):
        ...
        holder1 = Mock(state=State.offline, item=item1, _location=Location.ship, spec_set=Subsystem(1))
        holder2 = Mock(state=State.offline, item=item2, _location=Location.ship, spec_set=Subsystem(1))
        self.track_holder(holder1)
        self.track_holder(holder2)
        restriction_error1 = self.get_restriction_error(holder1, Restriction.subsystem_index)

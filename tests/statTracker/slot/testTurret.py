...
class TestTurretSlot(StatTestCase):
    ...
    def testTotal(self):
        ...
        shipItem = self.ch.type_(typeId=1, attributes={Attribute.turretSlotsLeft: 2})
        shipHolder = Mock(state=State.offline, item=shipItem, _location=None, spec_set=Ship)
        shipHolder.attributes = {Attribute.turretSlotsLeft: 6}
        self.setShip(shipHolder)
        self.assertEqual(self.st.turretSlots.total, 6)
        self.setShip(None)
    ...
    def testOutputNoShip(self):
        ...
        self.assertIsNone(self.st.turretSlots.total)
        self.assertEqual(len(self.log), 0)
        self.assertStatBuffersEmpty()
    ...
    def testUseEmpty(self):
        self.assertEqual(self.st.turretSlots.used, 0)
        self.assertEqual(len(self.log), 0)
        self.assertStatBuffersEmpty()
    ...
    def testUseSingle(self):
        ...
        self.assertEqual(self.st.turretSlots.used, 1)
    ...
    def testUseMultiple(self):
        ...
        self.assertEqual(self.st.turretSlots.used, 2)

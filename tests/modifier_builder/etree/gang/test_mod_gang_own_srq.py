...
class TestBuilderEtreeModGangOwnSrq(ModBuilderTestCase):
    ...
    def setUp(self):
        ...
        e_optr = self.ef.make(3, operandID=Operand.def_optr, expressionValue='PostMul')
        e_src_attr = self.ef.make(4, operandID=Operand.def_attr, expressionAttributeID=848)
        e_tgt_spec = self.ef.make(
            5, operandID=Operand.srq_attr,
            arg1=e_tgt_srq['expressionID'],
    ...
    def test_generic_build_success(self):
        ...
        self.assertEqual(modifier.src_attr, 848)
        self.assertEqual(modifier.operator, Operator.post_mul)
        self.assertEqual(modifier.tgt_attr, 654)

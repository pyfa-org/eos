...
class TestBuilderEtreeSelfType(ModBuilderTestCase):
    ...
    def make_etree(self, add_mod_operant, rm_mod_operand):
        e_tgt_own = self.ef.make(1, operandID=OperandId.def_dom, expressionValue='Char')
        e_self = self.ef.make(2, operandID=OperandId.def_dom, expressionValue='Self')
        e_self_type = self.ef.make(3, operandID=OperandId.get_type, arg1=e_self['expressionID'])
        e_tgt_attr = self.ef.make(4, operandID=OperandId.def_attr, expressionAttributeID=64)
        e_optr = self.ef.make(5, operandID=OperandId.def_optr, expressionValue='PostPercent')
        e_src_attr = self.ef.make(6, operandID=OperandId.def_attr, expressionAttributeID=292)
        e_tgt_itms = self.ef.make(
            7, operandID=OperandId.dom_srq,
            arg1=e_tgt_own['expressionID'],
            arg2=e_self_type['expressionID']
        )
        e_tgt_spec = self.ef.make(
            8, operandID=OperandId.itm_attr,
            arg1=e_tgt_itms['expressionID'],
            arg2=e_tgt_attr['expressionID']
        )
        e_optr_tgt = self.ef.make(
            9, operandID=OperandId.optr_tgt,

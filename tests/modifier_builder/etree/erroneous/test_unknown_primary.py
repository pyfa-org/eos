...
class TestBuilderEtreeErrorsUnknownPrimary(ModBuilderTestCase):
    ...
    def setUp(self):
        ...
        # In these expressions we use some arbitrary operands to make sure they fail.
        # They won't be root of the tree - we will get to them via splices, thus
        # they should not fail whole process of building modifiers
        self.e_mod_invalid = self.ef.make(
            9, operandID=Operand.def_grp,
            arg1=e_optr_tgt['expressionID'],
            arg2=e_src_attr['expressionID']
        )

    def test_partial_error_first(self):
        e_tgt = self.ef.make(10, operandID=Operand.def_dom, expressionValue='Ship')
        e_tgt_attr = self.ef.make(11, operandID=Operand.def_attr, expressionAttributeID=9)
        e_optr = self.ef.make(12, operandID=Operand.def_optr, expressionValue='PostPercent')
        e_src_attr = self.ef.make(13, operandID=Operand.def_attr, expressionAttributeID=327)
        e_tgt_spec = self.ef.make(
            14, operandID=Operand.itm_attr,
        ...
        e_rm_mod_valid = self.ef.make(
            17, operandID=Operand.rm_itm_mod,
            arg1=e_optr_tgt['expressionID'],
            arg2=e_src_attr['expressionID']
        )

...
class Operand(metaclass=Enum):
    ...
    add_itm_mod = 6
    # Applies modification to items belonging to some location, filtered by group,
    # format: ((location..groupFilter->targetAttribute).(operator)).ALGM(sourceAttribute)
    add_loc_grp_mod = 7
    # Applies modification to all items belonging to some location,
    # format: ((location->targetAttribute).(operator)).ALM(sourceAttribute)
    add_loc_mod = 8
    # Applies modification to items belonging to some location, filtered by skill requirement
    # format: ((location[skill_requirement]->targetAttribute).(operator)).ALRSM(sourceAttribute)
    add_loc_srq_mod = 9
    # Logical AND operator
    and_ = 10
    # Applies modification to items belonging to some location, filtered by owner of source,
    ...
    fof_launch = 47
    # Joins location and group definitions into single filter, format: location..group
    loc_grp = 48
    # Joins location and skill requirement definitions into single filter,
    ...
    rm_itm_mod = 58
    # Undos modification from items belonging to some location, filtered by group,
    # format: ((location..groupFilter->targetAttribute).(operator)).RLGM(sourceAttribute)
    rm_loc_grp_mod = 59
    # Undos modification from all items belonging to some location,
    # format: ((location->targetAttribute).(operator)).RLM(sourceAttribute)
    rm_loc_mod = 60
    # Undos modification from items belonging to some location, filtered by skill requirement,
    # format: ((location[skill_requirement]->targetAttribute).(operator)).RLRSM(sourceAttribute)
    rm_loc_srq_mod = 61
    # Undos modification from items belonging to some location, filtered by owner of source,

...
NORMALIZATION_MAP = {
    Operator.pre_assignment: lambda val: val,
    ...
    Operator.post_assignment: lambda val: val
}

# List operator types, according to their already normalized values
ASSIGNMENTS = (
    Operator.pre_assignment,
    Operator.post_assignment
)
ADDITIONS = (
    Operator.mod_add,
    Operator.mod_sub
...
class MutableAttributeMap:
    ...
    def __getitem__(self, attr_id):
        ...
        if attr_id == Attribute.skill_level:
        ...
        if self.__holder._fit is None:
            val = self.__holder.item.attributes[attr_id]
            return val
        ...
        # Else, we have to run full calculation process
            ...
            try:
                ...
                raise KeyError(attr_id) from e
            self.__holder._fit._link_tracker.clear_holder_attribute_dependents(self.__holder, attr_id)
        return val
    ...
    def __contains__(self, attr_id):
        ...
        result = attr_id in self.__modified_attributes or attr_id in self.__holder.item.attributes
        return result
    ...
    def __setitem__(self, attr_id, value):
        # Write value and clear all attributes relying on it
        self.__modified_attributes[attr_id] = value
        self.__holder._fit._link_tracker.clear_holder_attribute_dependents(self.__holder, attr_id)
    ...
    def __calculate(self, attr_id):
        ...
        try:
            attr_meta = self.__holder._fit.eos._cache_handler.get_attribute(attr_id)
        ...
        try:
            result = self.__holder.item.attributes[attr_id]
        ...
        for affector in self.__holder._fit._link_tracker.get_affectors(self.__holder, attr_id=attr_id):
            try:
                ...
                penalize = (attr_meta.stackable is False and
        ...
        if attr_meta.max_attribute_id is not None:
            try:
                max_value = self[attr_meta.max_attribute_id]
